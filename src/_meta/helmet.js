...
const extractHelmetNodes = (target, currentNode, dict = {}) => {
  ...
  const helmetNodes = content.reduce((nodeList, childNode) => {
    const { [HELMET_ATTRIBUTE]: key, ...attrs } = childNode.attrs || {};
  ...
  const result = {
    ...
    content: content
      .filter(
        ({ attrs }) => !(attrs && hasOwnProperty.call(attrs, HELMET_ATTRIBUTE)),
      )
...
export default (tree) => {
  ...
  const nodesToMove = [];
  tree.walk((node) => extractHelmetNodes(nodesToMove, node));
